...
const addData = () => {
  ...
  const [compteOwnerType,setCompteOwnerType] = useState('')
  ...
  const handleCompteOwner = (compteValue) => {
    setCompteOwnerType(compteValue)
  }
  ...
  // Get rider home location
  const getLocation = async () => {
    // Step 1: Provide a prominent disclosure
    Alert.alert(
      "مطلوب إذن الموقع",
      "يستخدم تطبيق Safe بيانات موقعك للمساعدة في حفظ عنوان منزل طفلك. يضمن ذلك توفير خدمات التوصيل والاستلام بدقة لطفلك. لن يتم جمع بيانات موقعك في الخلفية ولن يتم مشاركتها مع أطراف خارجية.",
      [
        {
          text: "Cancel",
          style: "cancel",
  ...
  const addNewRiderHandler = async () => {
    ...
    if (!user) return createAlert('المستخدم غير معرف');
    if (!studentFullName) return createAlert('يرجى ادخال الاسم الكامل');
    if (!dateSelected) return createAlert("يرجى إدخال تاريخ الميلاد");
    if (!studentSex) return createAlert("يرجى تحديد الجنس");
    if (!carType) return createAlert("يرجى اختيار نوع السيارة");
    if (!studentState) return createAlert("يرجى اختيار المحافظة");
    if (!studentCity) return createAlert("يرجى اختيار القضاء");
    if (!studentStreet) return createAlert("يرجى إدخال اسم الحي");
    if (!homeAdress) return createAlert("يرجى تحديد اقرب نقطة دالة على عنوانكم");
    if (!location) return createAlert("يرجى تحديد الموقع");
    if (!compteOwnerType) return createAlert("يرجى تحديد نوع الحساب");

    if( compteOwnerType === 'student') {
      if (!studentSchool) return createAlert("يرجى اختيار المدرسة");
    }

    if( compteOwnerType === 'employee') {
      if (!companyName) return createAlert("يرجى تحديد اسم مكان العمل");
      if (!companyAddress) return createAlert("يرجى تحديد عنوان مكان العمل");
      if (!companyNearestPoint) return createAlert("يرجى تحديد اقرب نقطة دالة على مكان عملكم");
    }
    ...
    try {
      const studentData = {
        ...
        destination:compteOwnerType === 'student' ? studentSchool : companyName,
        destination_location:compteOwnerType === 'student' ? schoolLocation : {latitude:0,longitude:0},
    ...
    switch (currentPage) {
        ...
        return (
          <View>
            <View style={styles.sectionLabelBox}>
              <Text style={styles.sectionLabelText}>معلومات المشترك الشخصية</Text>
            </View> 
            <TextInput
      ...
      case 2:
        return (
          <View>
            <View style={styles.sectionLabelBox}>
              <Text style={styles.sectionLabelText}>عنوان اقامة المشترك</Text>
            </View> 
      ...
      case 3:
        return (
          <View>
            <View style={styles.sectionLabelBox}>
              <Text style={styles.sectionLabelText}>صفة المشترك</Text>
            </View> 
            <Dropdown
              style={styles.dropdown}
              placeholderStyle={styles.dropdownStyle}
              ...
              value={compteOwnerType}
              onChange={item => handleCompteOwner(item.value)}
            />
            {compteOwnerType === 'student' && (
              <Dropdown
              style={styles.dropdown}
              placeholderStyle={styles.dropdownStyle}
              selectedTextStyle={styles.dropdownStyle}
      ...
      case 4:
        return (
          <View style={styles.worktimeContainer}>
            <View style={styles.sectionLabelBox}>
              <Text style={styles.sectionLabelText}>اوقات الدوام</Text>
            </View>  
  ...
  return(
    <SafeAreaView style={styles.container}>
      ...
      <View>
        <Text style={styles.title}>اضافة خط</Text>
        {renderPageIndicator()}
      </View>
      ...
      <View style={styles.BtnHalfContainer}>
        ...
        {currentPage < totalSteps ? (
          ...
          <TouchableOpacity style={styles.halfButton} onPress={addNewRiderHandler}>
